...
function activity(){
    ...
    $.ajax({
        url:'http://218.244.137.196:5000/json/activity',
        data:{
            start_day:start_day,
            end_day:end_day
        },
...
function hot(){
    var start_day=$('#start_time').val(),end_day=$('#end_time').val()
    $.ajax({
        url:'http://218.244.137.196:5000/json/hot',
        data:{
            start_day:start_day,
            end_day:end_day
        },
        ...
        success:function(data){
            ...
            $('.hot_shop').append("<tr><td></td><td>活动名称</td><td>城市</td><td>订单量</td></tr>")
            var t_body
