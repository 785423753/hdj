...
$(".cityPY li").on("click", this, function (event) {
    ...
    $(".list_city section").stop().animate({scrollTop: x[$(this).index()] - 130}, 1000)
});
...
$(window).scroll(function () {
    if ($(document).scrollTop() >= $(document).height() - $(window).height()) {
        ...
        var searchUrl="/tag/15/"+page;
         var listData = {city: h_city, cat: h_cat, month: parseInt(h_month), tag: h_tag, page: new_page};
        ...
        $.get(url, Data, function (data) {
            if (data && data.code == 1 && data.list.length > 0) {
                if($(".meeting").length==Data.page*10+2){
...
if ($("#list_nav").length != 0) {
    var nav_offset = $("#list_nav").offset();
    $(window).scroll(function () {
