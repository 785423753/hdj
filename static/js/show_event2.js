...
$(".bdsharebuttonbox a").css({
    ...
    $("#head_nav .nav_li").eq(0).addClass("active")
    $("#head_nav .nav_li").bind('click', function(event) {
    ...
    $("#free").bind('click',function(){
        ...
        $("body,html").animate({scrollTop:ordertop-92}, 600)
    })

    var class_top=[];
    $(window).scroll(function() {
        for (var i = 0; i <$(".content").length; i++) {
            class_top[i]=$(".content").eq(i).offset().top
        }
        if ( $(document).scrollTop()>=class_top[1]-104&&$(document).scrollTop()<class_top[2]-104) {
            $("#head_nav li").eq(1).addClass("active").siblings().removeClass("active")
        }else if ($(document).scrollTop()>=class_top[2]-104&&$(document).scrollTop()<class_top[3]-104) {
            $("#head_nav li").eq(2).addClass("active").siblings().removeClass("active")
        }else if ($(document).scrollTop()>=class_top[3]-104) {
            $("#head_nav li").eq(3).addClass("active").siblings().removeClass("active")
        }
        else{
            $("#head_nav li").eq(0).addClass("active").siblings().removeClass("active")
        }
    });


    var contentinfo = $(".content_body");
    contentinfo.find('table').width("100%")
