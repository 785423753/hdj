	...
	if($("#captcha").length>0) {
		$("#captcha").blur(function(){
			...
			if($.trim(value) != "" && $("#captcha").attr("btvd-el") == undefined) {
			  $.ajax({type:"post",url:url,data:{captcha:value,mobilphone:telephone},async:false,success:function(data){
				...
				if(tel!='' && (/^1[3-8]+\d{9}$/).test(tel)) {
					...
					$.get(url,{'tel':tel},function(data){
						...
						if(flag) {
						  ...
						  setTimeout(function(){
						  	$('#getCapcheByPhone').css({'color':'#428bca','cursor':'default'}).removeClass('disabled');
						  	$('#leftTime').remove();
						  	$('#miao').remove();
