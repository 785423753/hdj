...
$(function ($) {
	...
	$(".table_tr").find(".quantitys").live("change", function () {
			...
			$('.total_price').show();
            // $('.total_price').find('.total_price_num').html("总计:<span style='font-family:microsoft yahei;font-"
            //     +"size:24px;color:red;'>"+total_price+"</span>");

            // $('.total_price').find('.total_price_note').html("&lowast;付款后，你将获得"+returnpricess
            //     +"元现金奖励到你指定的支付宝账号");

            $('.total_price_num').html("<span style='color:#666666;'>共"+sum_nums+"张，</span>总计:</span>"
            ...
            $('.total_return_num').html("<span class='show_note glyphicon glyphicon-exclamation-sign' style='margin-right:10px;'>"
	                    +"<span class='show_note_font text-left' style='font-family:microsoft yahei;color:#666666;font-size:12px;position:absolute;z-index:100;margin-left:-164px;margin-top:24px;width:283px;height:95px;padding-left:10px;padding-top:10px;border:1px solid #ccc;background:#fcfcfc;'>"
