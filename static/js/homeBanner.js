...
var bannerIndex = 0;
bannner.css({transition:"all 1000ms ease",transform:"translate3d(-"+bannerIndex*870+"px, 0px, 0px)"});
...
var bannerTime = window.setInterval(function () {
    ...
    if(supportCss3("transform")){
        bannner.css({transform:"translate3d(-"+bannerIndex*870+"px, 0px, 0px)"});
        bannner.css({msTransform:"translate(-"+bannerIndex*870+"px, 0px)"});
    }else{
        $(".bannnerImg ul").animate({
            left:-bannerIndex*870+"px"
        },1000)
    }
    bannerIndex+=1;
},5000);
